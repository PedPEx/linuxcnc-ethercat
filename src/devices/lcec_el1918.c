...
///
/// Derived from lcec_el1918_logic.c. Identical behaviour, plus the 8 local
/// safe-input states read from the non-safe Device-Info PDO (0xf180) and
/// exported as local-in-0..7. Registered as type "EL1918".
///
/// local-in-* is a NON-SAFE diagnostic mirror of the terminal's local inputs
/// (for HMI / detailed messages such as "E-stop pressed"), never a safety
/// signal — safety decisions stay in the TwinSAFE program.

#include "lcec_el1918.h"

#include "../lcec.h"

static void lcec_el1918_read(lcec_slave_t *slave, long period);
static void lcec_el1918_write(lcec_slave_t *slave, long period);
static int lcec_el1918_preinit(lcec_slave_t *slave);
static int lcec_el1918_init(int comp_id, lcec_slave_t *slave);

static lcec_modparam_desc_t lcec_el1918_modparams[] = {
    {"fsoeSlaveIdx", LCEC_EL1918_PARAM_SLAVEID, MODPARAM_TYPE_U32},
    {"stdInName", LCEC_EL1918_PARAM_STDIN_NAME, MODPARAM_TYPE_STRING},
    {"stdOutName", LCEC_EL1918_PARAM_STDOUT_NAME, MODPARAM_TYPE_STRING},
    {NULL},
};

static lcec_typelist_t types[] = {
    {"EL1918", LCEC_BECKHOFF_VID, 0x077e3052, 1, lcec_el1918_preinit, lcec_el1918_init, lcec_el1918_modparams},
    {NULL},
};
ADD_TYPES(types);
...
typedef struct {
  ...
  lcec_el1918_fsoe_crc_t *fsoe_crc;
} lcec_el1918_fsoe_t;
...
typedef struct {
  ...
  hal_bit_t *std_in_pins[LCEC_EL1918_DIO_MAX_COUNT];
  int std_in_count;
  unsigned int std_in_os;

  hal_bit_t *std_out_pins[LCEC_EL1918_DIO_MAX_COUNT];
  int std_out_count;
  unsigned int std_out_os;

  // local safe inputs — non-safe diagnostic mirror of 0xf180.
  // Each channel is a bit pair: even bit = safe input state, odd bit = module fault.
  hal_bit_t *local_in_pins[LCEC_EL1918_LOCAL_CH_COUNT];
  hal_bit_t *local_fault_pins[LCEC_EL1918_LOCAL_CH_COUNT];
  unsigned int local_in_os;

  unsigned int state_os;
  unsigned int cycle_counter_os;

  // must be last entry (dynamic size)
  lcec_el1918_fsoe_t fsoe[];
} lcec_el1918_data_t;

static const lcec_pindesc_t slave_pins[] = {
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_data_t, state), "%s.%s.%s.state"},
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_data_t, cycle_counter), "%s.%s.%s.cycle-counter"},
    {HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL},
};

static const lcec_pindesc_t fsoe_pins[] = {
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_t, fsoe_master_cmd), "%s.%s.%s.fsoe-%d-master-cmd"},
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_t, fsoe_master_connid), "%s.%s.%s.fsoe-%d-master-connid"},
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_t, fsoe_slave_cmd), "%s.%s.%s.fsoe-%d-slave-cmd"},
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_t, fsoe_slave_connid), "%s.%s.%s.fsoe-%d-slave-connid"},
    {HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL},
};

static const lcec_pindesc_t fsoe_crc_pins[] = {
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_crc_t, fsoe_master_crc), "%s.%s.%s.fsoe-%d-master-crc"},
    {HAL_U32, HAL_OUT, offsetof(lcec_el1918_fsoe_crc_t, fsoe_slave_crc), "%s.%s.%s.fsoe-%d-slave-crc"},
    {HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL},
};
...
static int lcec_el1918_preinit(lcec_slave_t *slave) {
  ...
  for (p = slave->modparams; p != NULL && p->id >= 0; p++) {
    switch (p->id) {
      case LCEC_EL1918_PARAM_SLAVEID:
        // find slave
        index = p->value.u32;
      ...
      case LCEC_EL1918_PARAM_STDIN_NAME:
        stdin_count++;
        if (stdin_count > LCEC_EL1918_DIO_MAX_COUNT) {
      ...
      case LCEC_EL1918_PARAM_STDOUT_NAME:
        stdout_count++;
        if (stdout_count > LCEC_EL1918_DIO_MAX_COUNT) {
...
int lcec_el1918_init(int comp_id, lcec_slave_t *slave) {
  lcec_master_t *master = slave->master;
  lcec_el1918_data_t *hal_data;
  lcec_el1918_fsoe_t *fsoe_data;
  lcec_slave_modparam_t *p;
  int fsoe_idx, index, err, i;
  lcec_el1918_fsoe_crc_t *crc;
  lcec_slave_t *fsoe_slave;
  const LCEC_CONF_FSOE_T *fsoeConf;
  ...
  for (fsoe_idx = 0, p = slave->modparams; p != NULL && p->id >= 0; p++) {
    if (p->id == LCEC_EL1918_PARAM_SLAVEID) {
      fsoe_idx++;
    }
  }

  // alloc hal memory
  hal_data = LCEC_HAL_ALLOCATE_ARRAY(lcec_el1918_data_t, fsoe_idx * sizeof(lcec_el1918_fsoe_t));
  ...
  // local inputs: low 16 bits of the 24-bit 0xf180 object are PDO-mapped
  // (bits 0-7 = the 8 channels, 8-15 = per-channel diagnostics; verify in TC3).
  lcec_pdo_init(slave, 0xf180, 0x00, &hal_data->local_in_os, NULL);
  ...
  // export local input pins under the "local" subnode, numbered 1..8:
  //   local.in-N   = safe input state of channel N
  //   local.fault-N = module fault of channel N  (N = i + 1)
  for (i = 0; i < LCEC_EL1918_LOCAL_CH_COUNT; i++) {
    if ((err = lcec_pin_newf(HAL_BIT, HAL_OUT, (void **)&hal_data->local_in_pins[i], "%s.%s.%s.local.in-%d", LCEC_MODULE_NAME, master->name, slave->name, i + 1)) != 0) {
      return err;
    }
    if ((err = lcec_pin_newf(HAL_BIT, HAL_OUT, (void **)&hal_data->local_fault_pins[i], "%s.%s.%s.local.fault-%d", LCEC_MODULE_NAME, master->name, slave->name, i + 1)) != 0) {
      return err;
    }
  }

  // map and export stdios
  hal_data->std_in_count = export_std_pins(slave, LCEC_EL1918_PARAM_STDIN_NAME, hal_data->std_in_pins, HAL_IN);
  ...
  hal_data->std_out_count = export_std_pins(slave, LCEC_EL1918_PARAM_STDOUT_NAME, hal_data->std_out_pins, HAL_OUT);
  ...
  for (fsoe_idx = 0, fsoe_data = hal_data->fsoe, p = slave->modparams; p != NULL && p->id >= 0; p++) {
    if (p->id == LCEC_EL1918_PARAM_SLAVEID) {
      ...
      fsoe_data->fsoe_crc = LCEC_HAL_ALLOCATE_ARRAY(lcec_el1918_fsoe_crc_t, fsoeConf->data_channels);
...
void lcec_el1918_read(lcec_slave_t *slave, long period) {
  lcec_master_t *master = slave->master;
  ...
  // local safe inputs (non-safe diagnostic mirror)
  local_in = EC_READ_U16(&pd[hal_data->local_in_os]);
  for (i = 0; i < LCEC_EL1918_LOCAL_CH_COUNT; i++) {
    // bit 2*i = safe input state, bit 2*i+1 = module fault (per EL1918 doc)
    LCEC_PIN_BIT_SET(hal_data->local_in_pins[i], !!(local_in & (1 << (2 * i))));
    LCEC_PIN_BIT_SET(hal_data->local_fault_pins[i], !!(local_in & (1 << (2 * i + 1))));
  }
...
void lcec_el1918_write(lcec_slave_t *slave, long period) {
  lcec_master_t *master = slave->master;
  lcec_el1918_data_t *hal_data = (lcec_el1918_data_t *)slave->hal_data;
  uint8_t *pd = master->process_data;
  uint8_t std_in;
